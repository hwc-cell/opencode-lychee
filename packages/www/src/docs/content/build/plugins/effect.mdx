...
live-only, and close when Stream consumption stops. Durable definitions still use
the normal Bus sequence and persistence path; this API does not currently expose
replay. Method `errors` maps become typed Effect error channels. Construct one
