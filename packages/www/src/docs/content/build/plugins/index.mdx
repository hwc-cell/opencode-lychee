...
`metadata`. Event definitions may include `durable: { version, aggregate }`, where
`aggregate` names a string field in the schema output passed to Bus. Durable emits
use the normal Bus sequence and configured persistence flow, and subscribers also
receive `durable: { aggregateID, seq, version }`.

Subscriptions remain live-only: there is no plugin log/replay API yet, and events
while disconnected are missed even when they were published durably. The method
name `events` is reserved for the subclient's event API.
