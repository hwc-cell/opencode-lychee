...
const messages: Record<BridgeLang, Record<string, string>> = {
  zh: {
    ...
    cmdMissingLauncher: "未找到启动器, 请在仓库根目录执行: mkdir -p ~/.local/bin && ln -sfn \"$PWD/packages/opencode/lychee.sh\" ~/.local/bin/OpenCode-Lychee",
  ...
  en: {
    ...
    cmdMissingLauncher: "Launcher not found. From the repository root run: mkdir -p ~/.local/bin && ln -sfn \"$PWD/packages/opencode/lychee.sh\" ~/.local/bin/OpenCode-Lychee",
