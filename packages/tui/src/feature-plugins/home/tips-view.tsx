...
import { inputUndoTip, terminalSuspendTip, tips, t, type Shortcuts } from "../../i18n"
...
const NO_MODELS_TIP = t("tips.noModels")
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    inputUndo: useCommandShortcut("prompt.undo"),
  ...
  const tip = createMemo(() => {
    ...
    const list = [...tips, process.platform !== "win32" ? terminalSuspendTip : inputUndoTip].flatMap((item) => {
      const value = typeof item === "string" ? item : item(shortcuts)
      return value ? [value] : []
    })
    return list[Math.floor(tipOffset * list.length)] ?? NO_MODELS_TIP
  }, NO_MODELS_TIP)
