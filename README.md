...
<p align="center">
  ...
  <a href="#快速开始">快速开始</a> ·
  <a href="使用说明.md#tui-navigation">快捷键与跳转</a> ·
  <a href="使用说明.md#wechat">微信接入</a> ·
...
> 第一次使用建议直接阅读 **[《使用说明》](使用说明.md)**：从安装、连接模型到 TUI 跳转、主题切换、微信指令和常见问题都集中在一个页面。
...
| 微信聊天桥       | 扫码连接微信 Bot，支持流式回复、媒体附件、会话控制、切换模型和后台常驻             |
...
| 荔枝主题         | 提供荔枝红、晴空玻璃、荔枝汽水、青瓷、中国红和桂月等亮/暗色主题                    |
...
启动后不知道如何跳转？查看[快捷键与页面跳转](使用说明.md#tui-navigation)，或在 TUI 中按 `Ctrl+P` 打开命令面板。
...
| `/new` / `/clear`          | 立即新建会话，或清除当前会话映射                  |
| `/stop`                    | 停止当前正在运行的任务                            |
| `/status`                  | 查看模型、工作目录和会话状态                      |
| `/where [目录]`            | 查看或切换 AI 工作目录；切换操作仅限登录账号      |
| `/autostart` / `/autostop` | 开启或关闭后台常驻（仅扫码登录的账号可操作）      |
| `/help`                    | 查看帮助；旧拼写 `/halp` 仍兼容                   |

聊天桥支持文字、微信语音转写、图片、文件和视频附件。媒体只从受信任的微信 CDN 下载，在内存中完成 AES 解密后直接交给 OpenCode；默认单个附件上限为 10MB、单条消息总计 20MB，可通过 `LYCHEE_MEDIA_MAX_BYTES` 和 `LYCHEE_MEDIA_TOTAL_MAX_BYTES` 调整。

模型输出会增量发送，长任务默认每 5 分钟报告一次状态。新消息可以打断正在运行的任务；等待连接中断时会自动恢复。如果模型达到总等待上限，本轮会被停止并明确提示，不会重新提交整轮消息，从而避免工具操作重复执行。
