...
- 🚀 **快捷启动** —— 终端输入 `OpenCode-Lychee` 即可打开
...
```bash
OpenCode-Lychee weixin login       # ① 扫码登录(二维码自动续期; 登录成功直接进模型配置)
OpenCode-Lychee weixin configure   # ② (可选)重新配置默认模型与思考强度
OpenCode-Lychee weixin autostart   # ③ 装成系统常驻: 开机自启+崩溃重启
```

- 首次登录会直接进入默认模型配置;以后可在微信里随时发送 `/model` 查看/切换模型与思考强度(如 `/model deepseek v4 pro max`)
- `OpenCode-Lychee weixin status` 查看真实连接状态;`run` 临时前台运行;`autostop` 移除常驻;`logout` 清除登录与会话状态
